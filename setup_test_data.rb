...
if artist_app.save
  puts "✓ #{artist_app.new_record? ? 'Created' : 'Updated'} vendor application: Artists (ID: #{artist_app.id})"
else
...
if vendor_app.save
  puts "✓ #{vendor_app.new_record? ? 'Created' : 'Updated'} vendor application: Vendors (ID: #{vendor_app.id})"
else
